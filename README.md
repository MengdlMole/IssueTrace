...
- 可为事件设置多级分组、英文逗号分隔的多个标签和优先级；服务、版本号可复用历史输入，也允许录入新值。
- 待处理超过 30 分钟、处理中超过 60 分钟没有活动，自动进入“需要关注”。
- 可为当前问题设置 30 分钟、2 小时、明天此时或任意未来本地时间提醒；到时使用桌面通知，关闭窗口后可由系统托盘继续运行。
...
- “更多信息”中的事件描述支持文字、粘贴图片、选择图片和拖放图片，修改自动保存。
...
1. 按 `Ctrl/Cmd + N`，输入事件和提出人。尚未开始排查时点“先记下”；立即处理时点“保存并开始处理”。
...
7. 处理完点击“完成”，补充结论，然后点击“导出事件”选择目录。
