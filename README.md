...
| 事件详情 | 标题、文字描述和属性先在弹窗内编辑，只有点击“保存”才提交，“取消”不写入；可手工修正创建时间；提出人、处理人、版本号、服务、分组可选择完整历史值或输入新值 |
| 事件记录 | 普通记录、进展、结论三种类型；自动记录时间、倒序显示，可编辑内容和记录时间；最新进展立即同步到列表和详情 |
...
| 事件日历 | 月视图可在“按接收日”和“按处理区间”之间切换；后者会在创建日到完成日的每天显示事件，未完成事件延续到今天 |
| 回收站 | 事件和时间线记录删除前必须二次确认，删除后可在“工具 → 回收站”恢复；永久删除需再次确认，并清理所属附件文件 |
...
| 日历 | 用于按提出日期回顾，或查看从创建到完成的处理区间；不支持排期、拖动日期、周视图、工时日历或日程提醒 |
...
当前 macOS 开发构建的 13 项自动化测试全部通过，覆盖核心 CRUD、搜索筛选、迁移、XLSX/Markdown 导出、更新包边界、升级回滚、长页面滚动、状态即时刷新、自定义提醒、描述图片、计时修正、显式详情保存、回收站恢复与永久删除、记录时间修正、创建时间修正、日历区间投影、分组迁移、历史字段和三页面导航。自动化通过不等同于三个目标系统的人工发布验收。
...
7. 需要按日期复盘时打开“事件日历”：默认查看当天收到的事件，也可切换为“按处理区间”查看当天仍在处理的事件。
